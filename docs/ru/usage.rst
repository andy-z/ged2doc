...
может быть импортирована другим кодом Python. Основная функция |ged2doc| -
...
один файл соответствующий шаблону по умолчанию, тогда следует использовать опцию
``-p PATTERN`` для более точного соответствия, например::
...
HTML лучше подходит для онлайн-просмотра, так как включает в себя навигацию,
формат ODT лучше для печати (возможно после модификации в OpenOffice/LibreOffice).
...
то необходимо указать опцию ``-t`` или ``--type`` для указания формата документа::
...
Правильный файл GEDCOM должен иметь достаточную информацию в нем для определения
его кодировки. В некоторых случаях может потребоваться явно указать кодировку
файла или изменить способ обработки ошибок декодирования. По умолчанию |ged2doc|
пытается определить кодировку файла из содержимого файла и программа
останавливается при любых ошибках, связанных с кодировкой. Вы можете
использовать опцию ``--encoding`` чтобы использовать другую кодировку и
опцию ``-encoding-errors`` для управления обработкой ошибок. Аргументом опции
``--encoding`` является имя кодирования, такое как ``utf-8``, ``iso-8859-1``
и т. д. Аргументом для опции ``--encoding-errors`` является одно из ключевых
слов:
...
Ниже приведен пример команды, которая принудительно кодирует utf-8, но заменяет
неправильно закодированные данные::
...
``Y-M-D``
    Год, месяц и день, разделенные тире, например: ``2000-Дек-31``;
    ``2017-Дек``; ``2017``

``D-M-Y``
    День, месяц и год, разделенные тире, например: ``31-Дек-2000``;
    ``Дек-2017``; ``2017``

``Y/M/D``
    Год, месяц и день, разделенные косой чертой, например: ``2000/12/31``;
    ``2017/12``; ``2017``

``M/D/Y``
    Месяц, год и день, разделенные косой чертой, например: ``12/31/2000``;
    ``12/2017``; ``2017``.

``Y.M.D``
    Год, месяц и день, разделенные точкой, например: ``2000.12.31``;
    ``2017.12``; ``2017``

``D.M.Y``
    День, месяц и год, разделенные точкой, например: ``31.12.2000``;
    ``12.2017``; ``2017``. Этот формат используется по умолчанию для языка ``ru``.

``MD,Y``
    Запятая после дня, номер месяца, год, например: ``Dec 31, 2000``;
    ``Dec 2017``; ``2017``. Этот формат используется по умолчанию для языка ``en``.
...
найти его на диске), это можно отключить, используя опцию ``--no-image``,
...
В разных локальностях используются разные правила форматирования имени, которые
могут быть довольно сложными. По умолчанию |ged2doc| представляет имена людей
в качестве имени за которым следует фамилия (в браке) (например, "Дарья Иванова"),
но есть также несколько опций, которые могут изменить это представление:
...
--name-maiden         Фамилия в браке и фамилию до брака в скобках, например
                     ``Дарья Иванова (Курехина)``
--name-maiden-only    Фамилия до брака, например ``Дарья Курехина``
...
что-то вроде «ИВАНОВА (КУРЕХИНА), Дарья».

Specifying size in options
^^^^^^^^^^^^^^^^^^^^^^^^^^

Некоторые опции ниже принимают размеры как значение, размер может быть указан в
разных единицах. Единицы могут быть основаны на экранных координатах (пиксели)
...
``px``
    Размер задается в пикселях, обычно используется для экранных измерений,
    таких как HTML. Пример: ``100px``.

``pt``
    Размер задается в пойнтах, обычно используется для размеров печати,
    одна точка составляет 1/72 дюйма. Пример: ``100pt``.
...
``mm``
    ...
    1 in = 25.4 mm. Пример: ``100mm``.

``cm``
    Размер задается в сантиметрах, обычно используется для размеров печати,
    1 in = 2.54 cm. Пример: ``10cm``.

Параметры, которые принимают размер как значение, имеют тип единиц по умолчанию,
например, если тип единиц для опции - пиксели, значение ``300`` совпадает с
``300px``.

Опции HTML
^^^^^^^^^^

Несколько опций специфичны для выходного формата HTML:

--html-page-width SIZE    Ширина страницы HTML, единицы по умолчанию -
                          пиксели; значение по умолчанию: ``800px``
--html-image-width SIZE   Ширина изображения, единицы по умолчанию -
                          пиксели; значение по умолчанию: ``300px``
--html-image-height SIZE  Высота изображения, единицы по умолчанию - пиксели;
                          значение по умолчанию: ``300px``
...
Опции, специфичные для выходного формата ODT:

--odt-page-width SIZE    Ширина страницы, единица по умолчанию - дюймы;
                         значение по умолчанию: ``6in``
--odt-page-height SIZE   Высота страницы, единица по умолчанию - дюймы;
                         значение по умолчанию: ``9in``
--odt-margin-left SIZE   Отступ слева, единица по умолчанию - дюймы;
                         значение по умолчанию: ``0.5in``
--odt-margin-right SIZE  Отступ справа, единица по умолчанию - дюймы;
                         значение по умолчанию: ``0.5in``
--odt-margin-top SIZE    Отступ сверху, единица по умолчанию - дюймы;
                         значение по умолчанию: ``0.5in``
--odt-margin-bottom SIZE  Отступ снизу, единица по умолчанию - дюймы;
                         значение по умолчанию: ``0.25in``
--odt-image-width SIZE   Ширина изображения, единицы по умолчанию - дюймы;
                         значение по умолчанию: ``2in``
--odt-image-height SIZE  Высота изображения, единицы по умолчанию - дюймы;
                         значение по умолчанию: ``2in``
...
(фотографии и древовидные графики, являющиеся структурами SVG). Размер
...
публикации документа. Когда документ загружен в LibreOffice его содержание
необходимо обновить - перейдите к меню ``Tools``, затем ``Update`` и
``Indexes and Tables`` - это должно перестроить все ссылки в файле ODT.

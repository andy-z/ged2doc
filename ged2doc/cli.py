...
def _make_writer(argv: list[str] | None = None) -> tuple[argparse.Namespace, Writer]:
    ...
    parser.add_argument(
        ...
        action="count",
        default=0,
        help="Print some info to standard output, -vv prints debug info.",
    )
    parser.add_argument(
        "--log",
        default=None,
    ...
    parser.add_argument(
        "--version", action="version", version=version, help="Print version information and exit"
    )
    # TODO: can enable FileType when Python2 support is dropped
    parser.add_argument(
        "input",  # type=FileType(mode="rb",),
    ...
    group.add_argument("-i", "--image-path", metavar="PATH", help="Directory containing files with images")
    group.add_argument(
        "-p",
        "--file-name-pattern",
        metavar="PATTERN",
        default="*.ged*",
        help="Pattern to search for GEDCOM file inside ZIP archive, default: %(default)s",
    )
    group.add_argument("--encoding", help="Input file encoding, default is to guess from file contents")
    group.add_argument(
        "--encoding-errors",
        default="strict",
        metavar="MODE",
        help="Mode for handling decoding errors, one of strict, ignore, or replace; default: %(default)s",
    )
    ...
    group.add_argument(
        ...
        choices=["html", "odt"],
        help=(
            "Type of the output document, possible values:"
    ...
    group.add_argument(
        "-s",
        "--sort-order",
        default=NameOrder.SURNAME_GIVEN.value,
        metavar="ORDER",
        choices=NAME_ORDER_LIST,
        help="Ordering of the individuals, one of %(choices)s; default: %(default)s.",
    )
    group.add_argument(
        "--locale",
        default=None,
        metavar="LOCALE",
        help=("Locale name to use for name ordering, default is to use system locale."),
    )
    group.add_argument(
        "--no-missing-date",
        default=False,
    ...
    group.add_argument(
        "--no-image", default=False, action="store_true", help="Disable images in output document."
    )
    group.add_argument(
        "--no-toc", default=False, action="store_true", help="Disable Table of Contents in output document."
    )
    group.add_argument(
        "--no-stat", default=False, action="store_true", help="Disable Name Statistics in output document."
    )
    group.add_argument(
        "-w",
        "--tree-width",
        ...
        type=int,
        metavar="NUMBER",
        help="Number of generations in ancestors tree, default: %(default)s",
    )
    ...
    group.add_argument(
        ...
        dest="name_fmt",
        action="append_const",
        const=NameFormat.SURNAME_FIRST,
    ...
    group.add_argument(
        ...
        dest="name_fmt",
        action="append_const",
        const=NameFormat.COMMA,
        help="Format names with surname followed by comma (only if surname is in leading position).",
    )
    group.add_argument(
        "--name-maiden",
        dest="name_fmt",
        action="append_const",
        const=NameFormat.MAIDEN,
        help="Format names with surname followed by maiden name in parentheses.",
    )
    group.add_argument(
        "--name-maiden-only",
        dest="name_fmt",
        action="append_const",
        const=NameFormat.MAIDEN_ONLY,
    ...
    group.add_argument(
        ...
        dest="name_fmt",
        action="append_const",
        const=NameFormat.CAPITAL,
        help="Format names with surname and maiden name in all capital.",
    )
    ...
    group.add_argument(
        "-u", "--html-image-upscale", default=False, action="store_true", help="Upscale small images"
    )
    ...
    group.add_argument(
        ...
        metavar="SIZE",
        type=String2Size("in"),
        help="Page right margin in inches; default: %(default)s",
    )
    group.add_argument(
        "--odt-margin-top",
        default="0.5in",
    ...
    group.add_argument(
        ...
        metavar="SIZE",
        type=String2Size("in"),
        help="Page bottom margin in inches; default: %(default)s",
    )
    group.add_argument(
        "--odt-image-width",
        default="2in",
    ...
    logfmt = "%(levelname)s: %(name)s (%(filename)s:%(lineno)d) -- %(message)s"
    ...
    if args.type is None:
        ...
        if ext.lower() == ".odt":
            ...
            parser.error(
                "Cannot determine document type from file extension,"
